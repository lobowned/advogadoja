...
import OABVerificationModal from "@/components/OABVerificationModal";
    ...
    <>
      ...
      >
        {trustBadges.map((badge) => {
          ...
          const isClickable = badge.id === 'oab';
          ...
          return (
            <m.div
              ...
              className={`
                ...
                ${isClickable ? 'cursor-pointer ring-2 ring-transparent hover:ring-blue-500/50' : 'cursor-default'}
              `}
            >
              {/* Verified badge for OAB */}
              {isClickable && (
                <div className="absolute -top-1 -right-1 bg-blue-500 text-white text-[8px] px-1.5 py-0.5 rounded-full font-semibold shadow-sm">
                  VERIFICAR
                </div>
              )}
              ...
              <div className="
                ...
                hidden md:block
              ">
                {isClickable ? 'Clique para verificar' : badge.description}
      ...
      <OABVerificationModal isOpen={showOABModal} onClose={() => setShowOABModal(false)} />
    </>
  );
};
