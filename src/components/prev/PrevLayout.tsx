...
import { LAWYER, WHATSAPP_MESSAGES } from "@/lib/prev-config";
...
const NAV_ITEMS = [
  ...
  { label: "Maternidade", to: "/prev/salario-maternidade" },
  { label: "Sobre", to: "/prev/sobre" },
  { label: "Contato", to: "/prev/contato" },
];

interface PrevLayoutPropsExt extends PrevLayoutProps {
  quizKey?: string;
...
export default function PrevLayout({
  ...
  return (
    <div className={`prev-scope min-h-screen bg-prev-beige text-prev-navy antialiased${palette === "rose" ? " prev-rose-theme" : ""}`}>
      {/* ===== Header ===== */}
          ...
          <div className="lg:hidden border-t border-prev-navy/10 bg-prev-beige">
            <nav className="max-w-6xl mx-auto px-5 py-4 flex flex-col gap-1">
              ...
              <PrevWhatsappButton
                quizKey={quizKey}
                palette={palette}
                className="mt-3 text-center text-sm font-medium bg-prev-navy text-prev-beige px-5 py-3 rounded-full w-full"
              >
      ...
      {/* ===== Sticky CTA bar mobile (só aparece em < 768px) ===== */}
      <div className="prev-sticky-bar">
        <PrevWhatsappButton
          quizKey={quizKey}
          palette={palette}
          className="w-full inline-flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#1FB855] text-white py-3 rounded-full font-semibold text-sm shadow-lg"
      ...
      {/* ===== WhatsApp flutuante desktop (abre quiz) ===== */}
      <PrevWhatsappButton
        quizKey={quizKey}
        palette={palette}
        className="prev-floating-fab fixed bottom-6 right-6 z-50 group"
